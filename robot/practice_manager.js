...
💡 لطفاً در زمان مناسب تمرین خود را ارسال کنید.`;
  ...
  // بررسی اینکه آیا پیام تحلیل تمرین از مربی است
  isPracticeAnalysisMessage(message) {
    try {
      ...
      const isSchoolAdmin = USERS_BY_ROLE.SCHOOL_ADMIN.some(admin => 
        (typeof admin === 'object' ? admin.id : admin) === userId
      );
      const isCoach = USERS_BY_ROLE.COACH.some(coach => 
        (typeof coach === 'object' ? coach.id : coach) === userId
      );
      const isAssistant = USERS_BY_ROLE.ASSISTANT.some(assistant => 
        (typeof assistant === 'object' ? assistant.id : assistant) === userId
      );
      ...
      // بررسی اینکه آیا پیام اصلی تمرین است
  ...
  async handlePracticeAnalysis(message) {
    try {
      ...
      const studentName = originalMessage.from.first_name + (originalMessage.from.last_name ? ' ' + message.from.last_name : '');
      ...
      // ارسال تایید به مربی
      const coachMessage = `✅ تحلیل شما ثبت شد!

👤 دانشجو: ${studentName}
🎯 نوع: تحلیل صوتی
⏰ ${getTimeStamp()}

💡 تحلیل شما با موفقیت ذخیره شد.`;

      await sendMessage(chatId, coachMessage);

      // ارسال اطلاع به دانشجو
      const studentMessage = `🎯 تحلیل مربی دریافت شد!

👨‍🏫 مربی: ${coachName}
📝 نوع: تحلیل صوتی
⏰ ${getTimeStamp()}

💡 مربی شما تمرین شما را بررسی کرده است.`;

      await sendMessage(chatId, studentMessage);
